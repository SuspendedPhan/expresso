  ...
  <div class="absolute" ref="node" :style="style">
    <div :class="['border', 'border-black', { 'border-opacity-0': !selected }]">
      ...
      <Searchbox
        ...
        @queryInput="onSearchboxQueryInput"
        @choiceCommitted="onSearchboxChoiceCommitted"
      ></Searchbox>
    </div>
...
export default class WasmNode extends Vue {
  ...
  async mounted() {
    ...
    ;(window as any).wasmModule.EmbindUtil.setSignalListener(node.getOnChangedSignal(), () =>
      this.onNodeChanged()
    )
