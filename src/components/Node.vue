...
<template>
  <span class="node-root">
    ...
    <span v-if="astNode.value != null" @click="click" :class="{ highlighted }">{{
      astNode.value
    }}</span>
    <span v-else-if="astNode.metaname === 'Reference'" @click="click" :class="{ highlighted }">{{
      referenceToString(astNode)
    }}</span>
