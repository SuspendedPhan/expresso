...
export default class WasmAttribute extends Vue {
  ...
  @Provide() nodeLayout = new ElementLayout(
    () => this.getRootNode(),
    (node) => Store.getChildren(node),
    WasmAttribute.getKey,
    horizontalMargin,
    verticalMargin
  )
  ...
  async mounted() {
    ...
    if (this.attribute.constructor.name === 'EditableAttribute') {
      ...
      ;(window as any).wasmModule.EmbindUtil.setSignalListener(
        this.attribute.getOnChangedSignal(),
