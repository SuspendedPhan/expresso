    ...
    <div class="flex">
      <div class="w-1/2 h-full">
        ...
        <ProjectFunctionCollection
          v-if="projectFunctionsActive"
          :project="project"
          class="absolute bg-white border p-4 m-4 shadow-md border-black"
        />
      </div>
...
export default class WasmTest extends Vue {
  ...
  async mounted() {
    ...
    const render = (project, renderer) => {
      ...
      const timeAttribute = Functions.getAttributeByName(
        Functions.vectorToIterable(project.getRootOrganism().getAttributes()),
        'time'
      )
  ...
  private static addAverageFunction(module: any, project: any) {
    ...
    const primitiveFunctions = Functions.vectorToArray(
      module.PrimitiveFunctionCollection.getInstance().getFunctions()
    )
