...
export default class Dehydrator {
  public dehydrateLibraryProject$(
    ...
    const dehydratedLibraryProject$ = combineLatest([
      dehydratedProject$,
      libraryProject.name$,
    ]).pipe(
      map(([deProject, name]) => {
  ...
  public dehydrateProject$(project: Project): Observable<DehydratedProject> {
    ...
    const deProject$ = combineLatest([
      ...
      switchMap(async ([deExObjects, deComponents, deExFuncs]) => {
        const name = await DexRuntime.runPromise(
          Ref.get(project.libraryProject!.name)
        );
  ...
  dehydrateCloneNumberTarget(cloneNumberTarget: CloneNumberTarget) {
    log55.debug("Dehydrating CloneNumberTarget");
    const extracted: DehydratedCloneNumberTarget = {
