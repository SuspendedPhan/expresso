...
export default class Evaluator {
  ...
  public static *computeRenderCommands(): Iterable<any> {
    ...
    const windowCenterAttribute = Attribute.getAttributeFromName(organism, 'window.center')
    ...
    if (windowCenterAttribute) {
      windowCenterAttribute.assignVector(this.windowSize.width / 2, this.windowSize.height / 2)
    }
    if (time01Root) {
  ...
  private static *computeRenderCommandsForOrganism(organism): Iterable<any> {
    ...
    for (let cloneNumber = 0; cloneNumber < clones; cloneNumber++) {
      if (clonesRoot) {
        ...
        Evaluator.assignAttribute(organism, 'cloneNumber01', cloneNumber / (clones - 1))
        Evaluator.assignAttribute(organism, 'radialCloneNumber01', cloneNumber / clones)
      }
      ...
      if (metaorganism.renderShape !== RenderShape.None) {
        ...
        for (const attribute of this.attributeCollection.getEditables(organism)) {
          ...
          const value = this.evalNode(this.attributeCollection.getRootNode(attribute))
  ...
  private static assignAttribute(organism, attributeName: string, attributeValue: number) {
    const rootNode = this.attributeCollection.getRootNodeFromName(organism, attributeName)
