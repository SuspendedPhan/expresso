...
export class Layout {
  ...
  calculate(treeRoot): Output {
    ...
    this.calculateForChildren(treeRoot, localPositionsByKey, subtreeWidthsByKey)
    const lines = []
  ...
  private calculateLinesFromParentToChildren(
    ...
    for (const child of children) {
      ...
      lines.push(line)
      this.calculateLinesFromParentToChildren(child, childWorldPosition, localPositionsByKey, lines)
    }
  }
}
