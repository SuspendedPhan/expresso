...
export default class WasmPen {
  ...
  private static getNumberNodeChoices(query) {
    ...
    if (!Number.isNaN(number)) {
      return [
        { text: number, nodeMakerFunction: () => this.getModule().NumberNode.makeUnique(number) }
      ]
    } else {
      return []
    }
    ...
    const choices = [
      {
        text: 'Add',
        nodeMakerFunction: () =>
          this.getModule().AddOpNode.makeUnique(this.makeZero(), this.makeZero())
      },
      {
        text: 'Sub',
        nodeMakerFunction: () =>
          this.getModule().SubOpNode.makeUnique(this.makeZero(), this.makeZero())
      },
      {
        text: 'Mul',
        nodeMakerFunction: () =>
          this.getModule().MulOpNode.makeUnique(this.makeZero(), this.makeZero())
      },
      {
        text: 'Div',
        nodeMakerFunction: () =>
          this.getModule().DivOpNode.makeUnique(this.makeZero(), this.makeZero())
      },
      {
        text: 'Mod',
        nodeMakerFunction: () =>
          this.getModule().ModOpNode.makeUnique(this.makeZero(), this.makeZero())
      }
    ]
  ...
  private static getPrimitiveFunctionCallNodeChoices(query) {
    const primitiveFunctions = Functions.vectorToArray(
      this.getModule().PrimitiveFunctionCollection.getInstance().getFunctions()
    )
