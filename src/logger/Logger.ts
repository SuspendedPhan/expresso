import { FunctionCall, FunctionCallMetadata, LoggerDecorator } from "./LoggerDecorator";
...
export default class Logger {
    ...
    public static logCallstack() {
      ...
      const ancestors = this.getAncestors(LoggerDecorator.currentFunctionCall$.value);
    ...
    private static logFunctionCall(functionCall: FunctionCall) {
        functionCall.argsLogged$.subscribe((argsLogged) => {
            ...
            console.log(`${functionCall.metadata.className}.${functionCall.metadata.name}(${argString})`);
        });
    }
  }