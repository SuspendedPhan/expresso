...
func TestSimple(t *testing.T) {
	...
	length := expressor.Get("rootOrganismsRef").Get("value").Get("length").Int()
	assert.Equal(t, 0, length)

	expressor.Get("addOrganism").Invoke()
	length = expressor.Get("rootOrganismsRef").Get("value").Get("length").Int()
	assert.Equal(t, 1, length)
	name := expressor.Get("rootOrganismsRef").Get("value").Index(0).Get("name").String()
	assert.Equal(t, "organism0", name)

	expressor.Get("addOrganism").Invoke()
	length = expressor.Get("rootOrganismsRef").Get("value").Get("length").Int()
	assert.Equal(t, 2, length)
	name = expressor.Get("rootOrganismsRef").Get("value").Index(1).Get("name").String()
